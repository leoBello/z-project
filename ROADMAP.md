...
### Interface
- HUD : rappel des contrôles
- **Minimap** en haut à gauche : fond de carte pré-rendu avec ombrage de relief,
  position et cap du joueur, nom du biome courant
- Le type `MinimapMarker` est déjà en place pour afficher ennemis et objectifs
...
### Priorité 1 — boucle de jeu (prochaine étape)
- [ ] Jauge de vie : cœurs dans le HUD, branchés sur `useGameStore`
- [ ] Hitbox d'attaque : sphère devant le joueur pendant `ATTACK.hitWindow`
      (la fenêtre temporelle est déjà définie, il n'y a qu'à la brancher)
- [ ] Ennemi « Octorok » : statique ou en patrouille, tire un projectile
- [ ] Ennemi « Moblin » : détection dans un rayon, poursuite, corps-à-corps
- [ ] Machine à états d'IA (`EnemyState` est déjà typé)
- [ ] Dégâts au joueur, cooldown d'invincibilité, clignotement
- [ ] Écran de Game Over avec relance
- [ ] Marqueurs d'ennemis sur la minimap (`MinimapMarker` est prêt)
